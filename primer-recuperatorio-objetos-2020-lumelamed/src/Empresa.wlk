...
object empresaDeComerc {
	...
	method productoMasDescargado(unaFecha){
		const descargasQueOcurrieronEnXFecha = usuariosRegistrados.map { usuario => usuario.descargas() }.filter { descarga => descarga.fecha() == unaFecha }
																												//		 ^^^ ojo, descargas devuelve una lista, si mapeas una lista con este mensaje, vas a terminar con una lista de _listas_ de objetos. Acá querías usar flatMap
		if(descargasQueOcurrieronEnXFecha.isEmpty()){ // evito un posible error si esta lista esta vacia
			throw new Exception(message = "No ocurrieron descargas en la fecha indicada")		// max de una lista vacía rompe, así que no era necesario realmente
		}
